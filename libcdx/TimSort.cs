...
namespace libcdx
{
    ...
    * length of the array being sorted and the minimum merge sequence length. */
        ...
        private TimSort(T[] a, IComparer<T> c)
        {
            ...
            int len = a.Length;
            T[] newArray = new T[len < 2 * INITIAL_TMP_STORAGE_LENGTH ?  1 : INITIAL_TMP_STORAGE_LENGTH];
            tmp = newArray;
        ...
        private void Sort(T[] a, IComparer<T> c)
        {
            Sort(a, 0, a.Length, c);
        }

        private void Sort(T[] a, int lo, int hi, IComparer<T> c)
        {
            if (c == null)
            {
                Array.Sort(a, lo, hi);
        ...
        * @param c comparator to used for the sort */
        ...
        private static int GallopLeft(T key, T[] a, int baseInt, int len, int hint, IComparer<T> c)
        {
            if (DEBUG) {
            ...
            int lastOfs = 0;
            int ofs = 1;
            if (c.Compare(key, a[baseInt + hint]) > 0)
            {
                // Gallop right until a[base+hint+lastOfs] < key <= a[base+hint+ofs]
                int maxOfs = len - hint;
                while (ofs < maxOfs && c.Compare(key, a[baseInt + hint + ofs]) > 0)
                {
                    lastOfs = ofs;
                    ofs = (ofs << 1) + 1;
            ...
            while (lastOfs < ofs)
            {
                ...
                if (c.Compare(key, a[baseInt + m]) > 0)
        ...
        private static int GallopRight(T key, T[] a, int baseInt, int len, int hint, IComparer<T> c)
        {
            if (DEBUG) {
                Debug.Assert(len > 0 && hint >= 0 && hint < len);
            }
        ...
        private void MergeLo(int base1, int len1, int base2, int len2)
        {
            ...
            {
                throw new ArgumentOutOfRangeException("Comparison method violates its general contract!");
            }
            else
            {
                if (DEBUG) {
        ...
        {
            tmpCount = System.Math.Max(tmpCount, minCapacity);
