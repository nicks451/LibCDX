...
namespace libcdx.Utils
{
	 ...
	 * length of the array being sorted and the minimum merge sequence length. */
        ...
        private ComparableTimSort(Object[] a)
        {
            ...
            int len = a.Length;
            object[] newArray = new object[len < 2 * INITIAL_TMP_STORAGE_LENGTH ? (int)(uint)(len) >> 1 : INITIAL_TMP_STORAGE_LENGTH];
            tmp = newArray;
        ...
        private static void BinarySort(object[] a, int lo, int hi, int start)
        {
            ...
            for (; start < hi; start++)
            {
                IComparable<object> pivot = (IComparable<object>) a[start];
        ...
        private static int GallopLeft(IComparable key, Object[] a, int baseInt, int len, int hint)
        {
            if (DEBUG) {
            ...
            int lastOfs = 0;
            int ofs = 1;
            if (key.CompareTo(a[baseInt + hint]) > 0)
            {
                // Gallop right until a[base+hint+lastOfs] < key <= a[base+hint+ofs]
                int maxOfs = len - hint;
                while (ofs < maxOfs && key.CompareTo(a[baseInt + hint + ofs]) > 0)
                {
                    lastOfs = ofs;
                    ofs = (ofs << 1) + 1;
            ...
            while (lastOfs < ofs)
            {
                ...
                if (key.CompareTo(a[baseInt + m]) > 0)
        ...
        private static int GallopRight(IComparable key, Object[] a, int baseInt, int len, int hint)
        {
            if (DEBUG) {
                Debug.Assert(len > 0 && hint >= 0 && hint < len);
            }
